...
The final nonzero-origin real-NR Prepared hit/miss/restart-reuse regression is still pending in this checkpoint. The provider offset/cache-identity description above is supported by the implementation audit and does not substitute for that runtime result.
