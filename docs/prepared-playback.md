...
The bounded model-input policy uses implementation identity `frame-engine-prepared-v2;native-hdr-bounded-srgb-v1` and records `modelInputRange: bounded-sRGB-after-resample`. This prevents reuse of pixels prepared before the post-resize SDR bound was enforced, even with identical weights and user settings. Older segments remain stored under their original identities and follow the ordinary capacity policy.
...
Production segments are stored as 10-bit HEVC ([format](hdr-cache.md#hevc-storage)). A hit decodes the stored access unit with VideoToolbox, converts it on the GPU with the same importer an original miss uses, and writes a complete IOSurface-backed RGBA16F buffer. The reader keeps one decode position per lease, so sequential hits decode one frame and the first hit after a seek decodes from the segment's IDR. SHA-256 checks, full-segment validation on acquisition and individual payload validation on every read remain mandatory. The output lease owns this storage through GPU presentation, so the disk lease can be released afterward. Identities that name the Float32 policy still read Float32 pixels and pack them to RGBA16F with vImage on the CPU.
