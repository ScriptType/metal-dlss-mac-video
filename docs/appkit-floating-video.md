...
The scope is one observed other-application fullscreen Space round trip. It does not operate Mission Control or user applications/settings, identify arbitrary desktop IDs, certify every animation interval, or add physical input, HDR, A/V or pixel-equality evidence. The existing diagnostic modes and their preserved evidence remain separate.
