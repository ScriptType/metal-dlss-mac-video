...
`vf-command enhance policy live` requires 60 warmed completions in the current qualification epoch, after excluding its first three completions. The rolling p95 of the latest 60 completed durations must be at most 80% of the nominal source-frame period. These durations run from engine admission through completed output normalization; they are not physical presentation latency. The minimum processing shape is 320×192, with a model and positive strength. Tiny instrumentation shapes, strength-zero, bypass and Prepared paths cannot qualify. Qualification belongs to the current model, settings, source geometry and nominal rate, and Metal device; it never enters Live automatically. No current M3 neural configuration has been qualified as Live.

Seek, bypass and input geometry/colour resets discard qualification evidence. Replacing the filter starts a new session, so model/settings/device evidence cannot cross replacements. A changed nominal source rate starts a new qualification epoch without itself resetting engine temporal history. Each admitted frame retains its qualification epoch: an older completion is ignored for qualification, consumes neither cold nor warmed samples, and cannot invalidate newer evidence even if its duration is malformed. This also rejects old work when the rate changes A→B→A.
...
`scripts/test-mpv-live-policy.sh` compiles and runs the production policy helper's CPU tests without building or launching the player, shared engine or GPU runtime. Its six groups cover cold-sample exclusion, the inclusive 80% boundary, rolling p95 and fallback, explicit entry/re-entry, reset/replacement, rate epochs and stale completions, ineligible configurations, and malformed current inputs. Synthetic timings exercise policy decisions; they do not qualify a real model/device or demonstrate the integrated Live fallback clocks.

The [M3 policy regression](evidence/m3-live-policy.json) passes these six groups in optimized and UBSan builds, all 39 mpv tests, and the bundled app's 23 controls/lifecycle checks. ASan stalled during initialization before `main`; that unavailable result is retained. A visible 12.026-second PQ30 Adaptive run at 160×96 processing completed 121 warmed frames at 10.52 FPS, with two pending frames, no observed stale generations or drops, and six unchanged retained comparisons. It advanced 4.100 source seconds and buffered for 8.238 seconds. All 338 steady scheduled A/V samples met the 20-ms target. The ineligible processing shape still exported warmed count and p95 diagnostics; Live remained unavailable. This is a bounded integration regression, not real Live or long-duration/physical A/V qualification.
...
The HLG run exceeded the 20-ms target for one displayed frame at source PTS 4.583 seconds, observed in three consecutive samples. PQ variable-rate playback exceeded it at 8 seconds in four samples. Neither showed cumulative drift: the last-quarter median differed from the first by approximately −0.33 ms in every case. These are functional passes with two clock-target failures, so broader timing acceptance remains open. Those failures motivated the readiness checks below. No tested neural configuration qualified Live. The production-helper CPU tests cover qualification loss, fallback and recovery decisions, while actual qualified-Live deadline fallback remains unexercised. Adaptive deadline buffering is measured directly.
