...
It runs in ordinary mode on the 60-second, 30 fps PQ clip. It turns Prepared on, starts preparation and plays. Once playback advances, it samples the state every 100 ms for 12 seconds of wall time. It fails if the job leaves the preparing state, if the clocks buffer for enhancement or `buffer-count` rises, or if position advances outside 0.97 to 1.03 seconds per wall second. Then it switches enhancement off, checks that preparation continues, and switches it back on while playing. The report records frame drops, the worst lag of video behind wall time and the clock hold after switching back on, without asserting them.

On the M3 at 32 × 24, eleven runs advanced 0.995 to 1.004 seconds of video per wall second. No run buffered, `buffer-count` stayed flat, and each run dropped 0 to 2 frames. The worst lag stayed within 0.17 seconds, about the polling resolution. In the six runs that measured it, switching enhancement back on did not hold the clocks. Switching it on for the first time while a file plays builds the filter, and mpv held both clocks once, for 0 to 77 ms in three runs, until the first frame came through. With other work on the machine, one of ten earlier runs measured 0.94 after video froze for 0.75 seconds at the end of the window, and two fell up to 1.9 seconds behind before dropping about 100 frames to catch up. The clocks never paused. An 8-thread CPU load alone did not reproduce the lag.
