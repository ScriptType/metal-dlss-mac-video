...
| [#38](https://github.com/ScriptType/metal-dlss-mac-video/issues/38) Remove system PiP | The system PiP code still ships in `apps/macos/Sources` and runs only with `HDRPLAYER_ENABLE_PIP=1`. Apple DTS says sample-buffer PiP is iOS only. |
| [#17](https://github.com/ScriptType/metal-dlss-mac-video/issues/17) Floating video window | Not on `main`. Draft PR #22 has a prototype whose core is to be landed in a smaller PR. Replaces system PiP. |
| [#42](https://github.com/ScriptType/metal-dlss-mac-video/issues/42) Toolchain check and fresh build | `scripts/doctor.sh` detects a missing Metal Toolchain, but `scripts/check.sh` does not run it. `scripts/bootstrap.sh` calls `scripts/build-harness.sh` before any mpv build, so a fresh clone stops there. |
...
| [#41](https://github.com/ScriptType/metal-dlss-mac-video/issues/41) Drop rejected-optimization tests | The MLX-DLSS `hdr-player` pin still carries the test-only flow-output pooling and separable-erosion measurements. Their results stay in [benchmarks](m3-benchmarks.md). |
