...
| #7 Completed-work instrumentation | Two 300-frame M3 runs with 270 warmed samples each; matching numeric references; actual import/pack GPU times, completed wall stages, process/MLX samples and adapter hooks | Acceptance audit and a clean-revision baseline; broader adapter/power qualification is separate |
...
| #14 Prepared mode | Real NR cancel/resume and exact nine-frame boundary equivalence; mpv two-process neural cache reuse; original misses; Prepared DOM progress/capacity/cancel/resume/coverage checks | General decoder/container support, exact noncontiguous source timing and sustained clocked playback |
