...
| [#35](https://github.com/ScriptType/metal-dlss-mac-video/issues/35) 10-bit HEVC Prepared cache | Prepared segments are stored as 10-bit HEVC through VideoToolbox (`packages/FrameEngine/Sources/HDRCacheFrames.swift`). On the retained 1080p reference a prepared minute takes 57.4 MiB, so a 2-hour film needs 6.73 GiB of the default 8 GiB. Error against Float32 storage on the original path is p99 6.76 nits and max 140 nits ([cache format](hdr-cache.md#hevc-storage)). |
...
| [#17](https://github.com/ScriptType/metal-dlss-mac-video/issues/17) Floating video window | **View > Float Video** moves the playing video into an always-on-top panel and back without a reload (`apps/macos/Sources/FloatingVideoController.swift`). Every way out of the panel shows the main window with the video, and pauses if that window is on another Space. `scripts/test-floating-video.py` checks the moves, both close orders, another app's full-screen Space and quitting while floating. Keys in the panel still need the [human check](human-checks.md#floating-video-17). |
...
| [#39](https://github.com/ScriptType/metal-dlss-mac-video/issues/39) EDR headroom on screen change | `DisplayHeadroomMonitor` re-reads the video screen's headroom on screen and display-parameter changes and republishes it. mpv refreshes the enhanced layer's EDR metadata on both; the original PQ path's metadata belongs to MoltenVK. The display check is for a human (`docs/human-checks.md` step 7). |
